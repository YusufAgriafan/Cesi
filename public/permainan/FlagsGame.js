...
const drop = (e) => {
  ...
  if (isTouchDevice()) {
    ...
    if (
      ...
      currentDrop.insertAdjacentHTML(
        "afterbegin",
        `<img src="/permainan/${currentElement.id}.jpg">`
      );
      count += 1;
    }
  } else {
    ...
    if (draggedElementData === droppableElementData) {
      ...
      e.target.insertAdjacentHTML(
        "afterbegin",
        `<img src="/permainan/${draggedElementData}.jpg">`
      );
      count += 1;
    }
  }
  //Win
  if (count == 3) {
    result.innerText = `Selamat, Kamu Menang!`;
    stopGame();
  }
};
...
const creator = () => {
  ...
  for (let i of randomData) {
    ...
    flagDiv.innerHTML = `<img src="/permainan/${i}.jpg" id="${i}">`;
